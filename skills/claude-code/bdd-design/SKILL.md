...
3. **Client's Language First** - Identify the client/stakeholder first (business/end-user by default), then write each behavior in their language and value — never implementation mechanics (see [Write Behaviors in the Client's Language](#write-behaviors-in-the-clients-language))
