...
You are a normal Claude Code session that has just taken on a piece of real work. Unlike `ai-kanban-work-card` (where a card id is handed to you), **no card exists yet** — you create one to track your own work, drive it through the board, and hand it off for review when you finish.
...
- **Tags** — read the project's `.ai-rules.json` at the repo root and use its `scope` array verbatim as the card's tags (e.g. `["personal", "ai-kanban"]`). If there is no `.ai-rules.json`, or it has no `scope`, **ask the user** which tags to use.
- **Session id** — the value of the `$CLAUDE_CODE_SESSION_ID` environment variable, so the card carries a one-click `claude --resume` handle. Read it with your shell.
- **Task name** — a short, human title you infer from the work (e.g. "Add staled-card auto-park"). Used as both the card title and the renamed session label.

## Flow

Each step has its own instruction file in this skill's `steps/` directory — read and follow it:

1. **Open the card** — `steps/1-open-card.md`: gather tags + session id, infer a name, `create_card(...)` (it starts directly `in_progress`), announce it in one line, and rename the session to match.
...
**DO:**
- Track exactly one card per session — the work this session is doing.
...
- Silently continue untracked when the **critical** step fails — if `create_card` fails or the session id is missing, **stop and tell the user**; card creation is not a fire-and-forget side channel.
