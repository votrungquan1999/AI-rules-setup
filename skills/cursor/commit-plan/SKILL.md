...
0. **Work in the right repo first.** The repo being committed is often not the current dir — you might be in `~/git-repos/personal` while the work is in `quant-trading/`. Infer the repo from the conversation and the files being changed, and work from inside it; ask if the target repo is unclear.
