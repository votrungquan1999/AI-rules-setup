...
- Read your assigned entries from the `behavior-risks.md` artifact (the risk ids in your assignment) — read only those plus any they cross-reference.
- Read the **actual implemented code** for the paths those risks touch (from the `step-result.md` / `plan-steps.md` file lists and the plan).
...
Append each finding to the `adversarial-revalidation.md` artifact (one section per risk):
