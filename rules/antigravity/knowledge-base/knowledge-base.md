...
- **Capture when it's worth keeping.** After solving a non-obvious problem, finding a
  ...
  `ai-rules kb capture question` / `til` / `blueprint`. Do NOT capture trivial or
  one-off things — they only add review noise.
- **Memory is for always-true project facts only.** Use `ai-rules kb capture memory` sparingly
