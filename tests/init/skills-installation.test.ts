...
import { cleanupTestProject, createTestProject, fileExists, waitForAPIReady } from "../helpers/test-setup-utils";

describe("Skills Installation Tests", () => {
	 ...
	 * 2. Run CLI init with claude-code agent
	...
	it("should create correct directory structure for skills", async () => {
		...
		// Spawn CLI in interactive mode with claude-code agent specified
		const { child, result } = spawnCLI(["init", "--agent", "claude-code"], {
			cwd: testProjectDir,
			timeout: 30000,
		});
	...
	it("should install all available Claude Code skills", async () => {
		...
		expect(finalResult.stdout).toContain("Installing Claude Code skills");
	...
	it("should install skills alongside rules for Claude Code", async () => {
		...
		const skillsInstalled = EXPECTED_SKILLS.length;

		// Output should mention the number of skills
		expect(finalResult.stdout).toMatch(new RegExp(`${skillsInstalled}\\s+skill`, "i"));
	}, 30000);
});
