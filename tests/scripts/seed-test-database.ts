import { connectToTestDB, seedDatabase, cleanDatabase } from "../helpers/database-utils";
...
async function seedTestDatabase(): Promise<void> {
	...
	try {
		...
		const agents = fixtures.directoryContents["rules"] || [];
		...
		for (const agent of agents) {
			...
			const categories = fixtures.directoryContents[`rules/${agentName}`] || [];
			...
			for (const category of categories) {
				...
				const manifest = fixtures.manifests[manifestKey];
				...
				totalSeeded++;
				console.log(`✅ Seeded ${manifestKey} (${Object.keys(files).length} files)`);
			}
		}
